...
import { mkdirSync, writeFileSync, cpSync, rmSync, readdirSync, readFileSync, } from 'fs';
import { getConfig } from '@vercel/static-config';
function hashConfig(config) {
    const str = JSON.stringify(config);
    return Buffer.from(str).toString('base64url');
}
function flattenAndSort(object) {
    const sortedObject = {};
    const keys = [];
...
export function vercelPreset() {
    ...
    const injectVercelEntryServer = runOnce((reactRouterUserConfig) => {
        const appDirectory = reactRouterUserConfig.appDirectory ?? 'app';
        const entryServerFile = readdirSync(appDirectory).find(f => basename(f, extname(f)) === 'entry.server');
        if (entryServerFile) {
            originalEntryServerPath = join(appDirectory, entryServerFile);
            originalEntryServerContents = readFileSync(originalEntryServerPath, 'utf8');
            const usesVercelReactRouterPackage = /["']@vercel\/react-router['"]/.test(originalEntryServerContents);
            if (usesVercelReactRouterPackage) {
                console.log(`[vc] Detected "${entryServerFile}" using \`@vercel/react-router\``);
            }
            else {
                console.warn(`WARN: The \`@vercel/react-router\` package was not detected in your "${entryServerFile}" file.`);
                ...
                console.warn(`WARN: https://vercel.com/docs/frameworks/react-router#using-a-custom-app/entry.server-file`);
            }
        }
        else {
            ...
            cpSync(new URL('defaults/entry.server.jsx', import.meta.url), vercelEntryServerPath);
        }
    });
    const createServerBundles = (reactRouterUserConfig) => ({ branch }) => {
    ...
    const buildEnd = ({ buildManifest, reactRouterConfig, viteConfig, }) => {
    ...
    return {
        ...
        reactRouterConfig({ reactRouterUserConfig }) {
            return {
                 ...
                 */
                serverBundles: reactRouterUserConfig.ssr !== false
